...
export const GradeFilterContent = () => {
  ...
  useEffect(() => {
    if (isGradeSelected && selectedGrade) {
      const fetchGradeDistricts = async () => {
        try {
          const response = await getDistrictsByGrade(
          ...
          console.error(
            `${selectedGrade}등급 지역 데이터 가져오기 실패:`,
            error
          );
          ...
          >
            <div className="title">
              <img src={ddang} alt="땅땅이" className="ddang" />
              {
                GradeSubtitle(
                  displayGrade,
                  gradeDistrictInfo.names,
                  gradeDistrictInfo.count
                ).subtitle
              }
            </div>
  ...
  const defaultGradeInfo = GradeSubtitle(
    displayGrade,
    "반포2동, 대치1동, 상암동",
    15
  );
